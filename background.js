...
const DEFAULT_DB = {
  ...
  chrome.runtime.onInstalled.addListener(async () => {
    ...
    if (!db) {
      await chrome.storage.local.set({ db: DEFAULT_DB });
    }
    ensureContextMenu();
  });
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg?.type === 'db:get') {
      wrap(chrome.storage.local.get('db').then(({ db }) => db || DEFAULT_DB));
      return true;
    }
    if (msg?.type === 'req:create') {
      wrap(createRequirement(msg.title));
      return true;
    }
    if (msg?.type === 'req:select') {
      wrap(setLastSelected(msg.id));
      return true;
    }
    if (msg?.type === 'req:rename') {
      wrap(renameRequirement(msg.id, msg.title));
      return true;
    }
    if (msg?.type === 'req:archive') {
      wrap(setArchive(msg.id, true));
      return true;
    }
    if (msg?.type === 'req:unarchive') {
      wrap(setArchive(msg.id, false));
      return true;
    }
    if (msg?.type === 'req:delete') {
      wrap(deleteRequirement(msg.id));
      return true;
    }
    if (msg?.type === 'link:add') {
      wrap(addLinkToRequirement(msg.id, msg.link));
      return true;
    }
    if (msg?.type === 'link:remove') {
      wrap(removeLink(msg.id, msg.linkId));
      return true;
    }
  });
  ...
  async function createRequirement(title) {
    ...
    const req = { id, title: title?.trim() || '未命名需求', archived: false, createdAt: Date.now() };
  ...
  async function addLinkToRequirement(id, { title, url, favicon }) {
    ...
    const link = {
      ...
      title: (title || url || '未命名链接').slice(0, 200),
      url,
      favicon: favicon || '',
      addedAt: Date.now(),
    };
